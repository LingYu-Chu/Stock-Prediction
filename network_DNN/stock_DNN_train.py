...
# build model
model = Sequential()
model.add(Dense(units=128, activation='relu', input_dim=look_back))
model.add(Dense(units=1))
model.compile(loss='mse', optimizer='adam')

train_history = model.fit(trainX, trainY,
                          epochs=200, batch_size=6, shuffle=False,
                          validation_data=(testX, testY),
                          verbose=2)

test = model.evaluate(testX, testY, batch_size=6, verbose=2)
